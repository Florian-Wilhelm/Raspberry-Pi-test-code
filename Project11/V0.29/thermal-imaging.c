      ...
      */
      ...
      while(true){   
           ...
           { 
             ...
             TemperatureRaw = (pixelValue[0] << 8) + pixelValue[1]; 
             
             // data is stored in two's complement format what has to be taken into account
             if (TemperatureRaw < 0)     
                decimalValueTemperature = (-1)*((~TemperatureRaw)+1) / 50;
             else 
                decimalValueTemperature = TemperatureRaw / 50;    
             ...
             sleep_ms(1);
              ...
              pixelAddress = pixelAddress + 0x04;
