/**
 * 
 * Test code as of October 19, 2025, created by FWD
 * 
...
short TemperatureRawRow1[15]; 
short TemperatureRawRow2[15]; 
short TemperatureRawRow3[15]; 
short TemperatureRawRow4[15];

short decimalValueTemperatureRow1[15]; 
short decimalValueTemperatureRow2[15]; 
short decimalValueTemperatureRow3[15]; 
short decimalValueTemperatureRow4[15]; 
...
{  
    ...
    connect();
    http_set_ssi_handler(mySSIHandler, ssitags, 60);
    httpd_init();
    ...
    /*
    Playing around with the temperature thresholds via attached dip switch
    */
    uint32_t mask = (1 << DIP_SWITCH_PIN2 | 1 << DIP_SWITCH_PIN3);
    
    gpio_init_mask(mask);
    gpio_set_dir_masked(mask, GPIO_IN);
    gpio_pull_up(DIP_SWITCH_PIN2);
    gpio_pull_up(DIP_SWITCH_PIN3);
    
    if (!gpio_get(DIP_SWITCH_PIN2)){
      thresholdUp = 30; 
      thresholdDown = 28; 
    }
    
    else if (!gpio_get(DIP_SWITCH_PIN3)){
      thresholdUp = 28; 
      thresholdDown = 26; 
    }    
    ...
    */
      ...
      i2c_write_blocking(I2C_ID, 0x29, TSL25911_LB_CH1, 1, false);    
      i2c_read_blocking(I2C_ID, 0x29, TSL25911_LB_CH1, 1, false);
      
      uint8_t TSL25911_HB_CH1[] = {0xB7};
    
      i2c_write_blocking(I2C_ID, 0x29, TSL25911_HB_CH1, 1, false);    
      ...
      uart_write_blocking(UART_ID, readValue, strlen(readValue));   
      sprintf(readValue, "TSL25911 CH1: %x %x \r\n", TSL25911_HB_CH1[0], TSL25911_LB_CH1[0]);  
      uart_write_blocking(UART_ID, readValue, strlen(readValue)); 
      ...
      while(true){   
           ...
           { 
             ...
             if (TemperatureRawRow1[i-1] < 0)     
                decimalValueTemperatureRow1[i-1] = (-1)*((~TemperatureRawRow1[i-1])+1) / 50;
             else 
                decimalValueTemperatureRow1[i-1] = TemperatureRawRow1[i-1] / 50;
                             
             sleep_ms(1);
             
              // 1st display line        
              if (decimalValueTemperatureRow1[i-1] > thresholdUp)
                renderImage(0, (0+i*8), (7+i*8), 0, 0); 
              else if (decimalValueTemperatureRow1[i-1] < thresholdDown)
           ...
           { 
             ...
             if (TemperatureRawRow2[i-1] < 0)     
                decimalValueTemperatureRow2[i-1] = (-1)*((~TemperatureRawRow2[i-1])+1) / 50;
             else 
                decimalValueTemperatureRow2[i-1] = TemperatureRawRow2[i-1] / 50;
                             
             sleep_ms(1);
             
              // 2nd display line        
              if (decimalValueTemperatureRow2[i-1] > thresholdUp)
                renderImage(0, (0+i*8), (7+i*8), 1, 1); 
              else if (decimalValueTemperatureRow2[i-1] < thresholdDown)
           ...
           { 
             ...
             if (TemperatureRawRow3[i-1] < 0)     
                decimalValueTemperatureRow3[i-1] = (-1)*((~TemperatureRawRow3[i-1])+1) / 50;
             else 
                decimalValueTemperatureRow3[i-1] = TemperatureRawRow3[i-1] / 50;
                             
             sleep_ms(1);
             
              // 3rd display line       
              if (decimalValueTemperatureRow3[i-1] > thresholdUp)
                renderImage(0, (0+i*8), (7+i*8), 2, 2); 
              else if (decimalValueTemperatureRow3[i-1] < thresholdDown)
           ...
           { 
             ...
             if (TemperatureRawRow4[i-1] < 0)     
                decimalValueTemperatureRow4[i-1] = (-1)*((~TemperatureRawRow4[i-1])+1) / 50;
             else 
                decimalValueTemperatureRow4[i-1] = TemperatureRawRow4[i-1] / 50;
                             
             sleep_ms(1);
             
              // 4th display line       
              if (decimalValueTemperatureRow4[i-1] > thresholdUp)
                renderImage(0, (0+i*8), (7+i*8), 3, 3); 
              else if (decimalValueTemperatureRow4[i-1] < thresholdDown)
